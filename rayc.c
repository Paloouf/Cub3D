...
/*   Updated: 2023/10/11 11:03:59 by ltressen         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	init_game(t_cub *data)
{
	...
	+ (180 * (data->map[y][x] == 'S')) + (270 * (data->map[y][x] == 'W')));
	data->pvangle = 0;
...
void	camera(t_cub *cub, int x)
{
	...
	cub->cam[x].tex_pos = (cub->cam[x].draw_start - (HEIGHT / 2)
		+ (cub->cam[x].line_height / 2)) * cub->cam[x].step;
	i = 0;
	while (i < HEIGHT)
	{
		...
		else
		{
			...
			* cub->tex[cub->cam[x].tex_num].img_h)) + cub->cam[x].tex_X]);
		}
		i++;
	}
}
void	game_win(t_cub *cub)
{
	...
	while (y < (HEIGHT))
	{
		...
		while (x < WIDTH)
		{
			...
			/ WIDTH)))) + (((int)(y * ((double)cub->tex[14].img_h / (HEIGHT))))
			* cub->tex[14].img_w))];
...
void	game_over(t_cub *cub)
{
	...
	while (y < (HEIGHT))
	{
		...
		while (x < WIDTH)
		{
			...
			/ WIDTH)))) + (((int)(y * ((double)cub->tex[13].img_h / (HEIGHT))))
			* cub->tex[13].img_w))];
			pxl_to_img(cub, x, y, color);
			x++;
		}
...
void	check_sprite(t_cub *cub)
{
	...
	while (i < cub->tono)
	{
		...
		- cub->spr[i].x)) + ((cub->posY - cub->spr[i].y) * (cub->posY
		- cub->spr[i].y)));
...
void	menu(t_cub *cub)
{
	...
	while (y < (HEIGHT))
	{
		...
		while (x < WIDTH)
		{
			...
			/ WIDTH)))) + (((int)(y * ((double)cub->tex[12].img_h / (HEIGHT))))
			* cub->tex[12].img_w))];
			pxl_to_img(cub, x, y, color);
			x++;
		}
				...
				* cub->tex[cub->spr[cub->spr_order[i]].tex].img_w
				 ...
				 && cub->spr[cub->spr_order[i]].transY < cub->cam[j].w_dist)
				{
					...
					while (k < cub->spr[cub->spr_order[i]].draw_endY)
					{
						 ...
						 * 128) + cub->spr[cub->spr_order[i]].spr_hgt * 128)
						 * cub->tex[cub->spr[cub->spr_order[i]].tex].img_h)
