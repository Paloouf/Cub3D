...
void	move (t_cub *cub)
{
	...
	if (cub->key.back == 1 && cub->map[(int)(cub->posY - (cub->dirY / 5))][(int)cub->posX] != '1' && cub->map[(int)cub->posY][(int)(cub->posX - (cub->dirX / 5))] != '1')
	{
		cub->posX -= cub->dirX / 10;
		cub->posY -= cub->dirY / 10;
	}
	if (cub->key.s_right == 1 && cub->map[(int)(cub->posY + (cub->dirX / 5))][(int)cub->posX] != '1' && cub->map[(int)cub->posY][(int)(cub->posX - (cub->dirY / 5))] != '1')
	{
		cub->posX -= cub->dirY / 10;
		cub->posY += cub->dirX / 10;
	}
	if (cub->key.s_left == 1 && cub->map[(int)(cub->posY - (cub->dirX / 5))][(int)cub->posX] != '1' && cub->map[(int)cub->posY][(int)(cub->posX + (cub->dirY / 5))] != '1')
	{
		cub->posX += cub->dirY / 10;
		cub->posY -= cub->dirX / 10;
	}
